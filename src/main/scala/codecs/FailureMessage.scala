...
case object TemporaryNodeFailure extends Node {
  def message = "TemporaryNodeFailure"
}
case object PermanentNodeFailure extends Perm with Node {
  def message = "PermanentNodeFailure"
}
case object RequiredNodeFeatureMissing extends Perm with Node {
  def message = "RequiredNodeFeatureMissing"
}
case class InvalidOnionVersion(onionHash: ByteVector32)
    extends BadOnion
    with Perm {
  def message = "InvalidOnionVersion"
}
case class InvalidOnionHmac(onionHash: ByteVector32)
    extends BadOnion
    with Perm {
  def message = "InvalidOnionHmac"
}
case class InvalidOnionKey(onionHash: ByteVector32) extends BadOnion with Perm {
  def message = "InvalidOnionKey"
}
case class TemporaryChannelFailure(update: ChannelUpdate) extends Update {
  def message = "TemporaryChannelFailure"
}
case object PermanentChannelFailure extends Perm {
  def message = "PermanentChannelFailure"
}
case object RequiredChannelFeatureMissing extends Perm {
  def message = "RequiredChannelFeatureMissing"
}
case object UnknownNextPeer extends Perm { def message = "UnknownNextPeer" }
case class AmountBelowMinimum(amount: MilliSatoshi, update: ChannelUpdate)
    extends Update { def message = "AmountBelowMinimum" }
case class FeeInsufficient(amount: MilliSatoshi, update: ChannelUpdate)
    extends Update { def message = "FeeInsufficient" }
case object TrampolineFeeInsufficient extends Node {
  def message = "TrampolineFeeInsufficient"
}
case class ChannelDisabled(
    messageFlags: Byte,
    channelFlags: Byte,
    update: ChannelUpdate
) extends Update { def message = "ChannelDisabled" }
case class IncorrectCltvExpiry(expiry: CltvExpiry, update: ChannelUpdate)
    extends Update { def message = "IncorrectCltvExpiry" }
case class IncorrectOrUnknownPaymentDetails(amount: MilliSatoshi, height: Long)
    extends Perm { def message = "IncorrectOrUnknownPaymentDetails" }
case class ExpiryTooSoon(update: ChannelUpdate) extends Update {
  def message = "ExpiryTooSoon"
}
case object TrampolineExpiryTooSoon extends Node {
  def message = "TrampolineExpiryTooSoon"
}
case class FinalIncorrectCltvExpiry(expiry: CltvExpiry) extends FailureMessage {
  def message = "FinalIncorrectCltvExpiry"
}
case class FinalIncorrectHtlcAmount(amount: MilliSatoshi)
    extends FailureMessage { def message = "FinalIncorrectHtlcAmount" }
case object ExpiryTooFar extends FailureMessage { def message = "ExpiryTooFar" }
case class InvalidOnionPayload(tag: ULong, offset: Int) extends Perm {
  def message = "InvalidOnionPayload"
}
case object PaymentTimeout extends FailureMessage {
  def message = "PaymentTimeout"
}

/** We allow remote nodes to send us unknown failure codes (e.g. deprecated
  * failure codes). By reading the PERM and NODE bits we can still extract
  * useful information for payment retry even without knowing how to decode the
  * failure payload (but we can't extract a channel update or onion hash).
  */
...
object FailureMessageCodecs {
  ...
  val failureMessageCodec = discriminatorWithDefault(
    discriminated[FailureMessage]
      ...
      .typecase(
        UPDATE | 11,
        (("amountMsat" | millisatoshi) :: ("channelUpdate" | channelUpdateCodec))
      ...
      .typecase(
        UPDATE | 13,
        (("expiry" | cltvExpiry) :: ("channelUpdate" | channelUpdateCodec))
          .as[IncorrectCltvExpiry]
      )
      .typecase(
        UPDATE | 14,
        ("channelUpdate" | channelUpdateCodec).as[ExpiryTooSoon]
      )
      .typecase(
        UPDATE | 20,
        (("messageFlags" | byte) :: ("channelFlags" | byte) :: ("channelUpdate" | channelUpdateCodec))
          .as[ChannelDisabled]
      )
      .typecase(
        PERM | 15,
    ...
    uint16.xmap(
      code => {
        val failureMessage = code match {
          case fc if (fc & PERM) != 0 && (fc & NODE) != 0 =>
            new UnknownFailureMessage with Perm with Node {
              override lazy val code = fc
            }
          case fc if (fc & NODE) != 0 =>
            new UnknownFailureMessage with Node { override lazy val code = fc }
          case fc if (fc & PERM) != 0 =>
            new UnknownFailureMessage with Perm { override lazy val code = fc }
  ...
  /** An onion-encrypted failure from an intermediate node:
    ...
    */
  def failureOnionCodec(mac: Hmac256): Codec[FailureMessage] =
