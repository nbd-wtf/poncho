...
object Sphinx {
  ...
  def generateStream(key: ByteVector, length: Int): ByteVector =
    ByteVector(
      ChaCha20
        .xor(
          Array.fill[UByte](length)(0.toUByte),
  ...
  def peel(
    ...
    case 0 =>
      val sharedSecret = computeSharedSecret(packet.publicKey, privateKey)
      val mu = generateKey("mu", sharedSecret)
      val check = Crypto.hmac256(
        mu,
      ...
      if (check == packet.hmac) {
        ...
        val nextPubKey = blind(
          packet.publicKey,
          computeBlindingFactor(packet.publicKey, sharedSecret)
        )
  ...
  object RouteBlinding {
    ...
    def create(
      ...
      val (blindedHops, blindingKeys) = publicKeys
        ...
        .map { case (publicKey, payload) =>
          val blindingKey = Crypto.getPublicKey(e)
          ...
          val rho = generateKey("rho", sharedSecret)
          val encrypted = ByteVector(
            ChaCha20Poly1305
              .encrypt(
                rho.toArray.map[UByte](_.toUByte),
                Array.fill[UByte](12)(0.toUByte),
                payload.toArray.map[UByte](_.toUByte)
              )
              .map[Byte](_.toByte)
          )
          e = Crypto.multiplyPrivateKey(
            e,
            Crypto.sha256(blindingKey ++ sharedSecret.bytes)
          )
    ...
    def derivePrivateKey(
      ...
      Crypto.multiplyPrivateKey(
        privateKey,
        generateKey("blinded_node_id", sharedSecret)
      )
    }
    ...
    def decryptPayload(
      ...
      val decrypted =
        ByteVector(
          ChaCha20Poly1305
            .decrypt(
              encryptedPayload.toArray.map[UByte](_.toUByte),
              rho.toArray.map[UByte](_.toUByte),
              Array.fill[UByte](12)(0.toUByte)
            )
            .get
